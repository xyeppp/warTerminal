...
function WarCache.Collect()
	...
	local language = Languages[SystemData.Settings.Language.active]
	...
	for _,data in pairs(EA_Window_InteractionSpecialtyTraining.advanceData) do
		if data.abilityInfo then
			...
			abilityData.tooltip[language].name = GetStringFormat(StringTables.Default.LABEL_ABILITY_TOOLTIP_ABILITY_NAME, {data.abilityInfo.name})
			abilityData.tooltip[language].descriptions = {}
			for dindex = 1, 40
			do
				abilityData.tooltip[language].descriptions[dindex] = GetAbilityDesc (data.abilityInfo.id, dindex)
			end
			abilityData.tooltip[language].specline = DataUtils.GetAbilitySpecLine (data.abilityInfo)
			abilityData.tooltip[language].cost = WarCache.GetAbilityCostText (data.abilityInfo)
			abilityData.tooltip[language].casttime = WarCache.GetAbilityCastTimeText (data.abilityInfo)
			abilityData.tooltip[language].type = DataUtils.GetAbilityTypeText (data.abilityInfo)
			abilityData.tooltip[language].level = WarCache.GetAbilityLevelText (data.abilityInfo)
			abilityData.tooltip[language].range = WarCache.GetAbilityRangeText (data.abilityInfo)
			local realCooldown = GetAbilityCooldown( data.abilityInfo.id ) / 1000
			abilityData.tooltip[language].cooldown = WarCache.GetAbilityCooldownText( realCooldown )
			local reqs = {}
